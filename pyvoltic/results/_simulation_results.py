...
class DFDResults(SimResults):
    ...
    def full_simulation(self,figsize  = (6,3)):
        fig = plt.figure(figsize = figsize, dpi = 200)
        ax = plt.subplot(ylim = [-0.05, 1.05])
        ax.plot(self.output[:,0], label = r'$\theta$', linestyle = 'dashed', color = 'grey', linewidth = 2)
        ...
        ax.plot(self.output[:,4], label = 'R', color = '#33A02C', linestyle = 'dashed', linewidth = 2)
        ax.plot(self.output[:,5], label = 'S',color = '#003087', linestyle = 'dashed', linewidth = 2)
        ax.plot(self.output[:,6], label = 'I', color = 'red', linestyle = 'dashed', linewidth = 2)
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        ax.set_ylabel('Proportion\n')
        plt.tight_layout()
