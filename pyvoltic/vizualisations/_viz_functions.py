...
def NE_multi_cumulative_incidence(
    output_list: list,
    title: str,
    label_list: list,
    line_style: list,
    ...
    for output in output_list:
        if output.shape[1] != 6:
            raise ValueError(
                "Incorrect Simulation Output, Results are derived from the SIRNE class"
            )
...
def SR_multi_cumulative_incidence(
    output_list: list,
    title: str,
    label_list: list,
    line_style: list,
    ...
    for output in output_list:
        if output.shape[1] != 5:
            raise ValueError(
                "Incorrect Simulation Output, Results are derived from the SIRSR class"
            )
    fig, ax = plt.subplots(figsize=figsize, dpi=200)
    ax.set_ylim(-0.05, 1.05)
    for i, output in enumerate(output_list):
        ax.plot(
            1 - (output[:, 4] + output[:, 3]) + output[:, 4],
            alpha=0.5,
            c=color,
            label=label_list[i],
            ls=line_style[i],
...
def EBCM_multi_cumulative_incidence(
    output_list: list,
    title: str,
    label_list: list,
    line_style: list,
    ...
    for output in output_list:
        if output.shape[1] != 4:
            raise ValueError(
                "Incorrect Simulation Output, Results are derived from the EBMC/MFSH class"
            )
    fig, ax = plt.subplots(figsize=figsize, dpi=200)
    ax.set_ylim(-0.05, 1.05)
    for i, output in enumerate(output_list):
...
def _NE_plot_full_graph(
    output: np.array, title: str = "", figsize: tuple = (12, 5), log_scale: bool = False
):
    if output.shape[1] != 6:
        raise ValueError(
            "Incorrect Simulation Output, Results are derived from the SIRNE class"
    ...
    fig, ax = plt.subplots(figsize=figsize)
    ax.plot(
        output[:, 0], alpha=0.5, label="Fraction degree 1 nodes sus"
    )  # change of theta
    ax.plot(
        output[:, 1],
    ...
    ax.plot(
        output[:, 4], label="Infectious ego with an alter of any state"
    )  # change of M_I
